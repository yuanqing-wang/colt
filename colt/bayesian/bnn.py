...
import dgl
import functools
from ..representation.gnn import GNN
...
class UnwrappedBNNModel(torch.nn.Module):
    ...
    def forward(self, x, y=None):        
        if isinstance(x, dgl.DGLGraph):
            y_hat = self.representation(x, x.ndata["h"])
        else:
            y_hat = self.representation(x)
        y_hat_mu, y_hat_log_sigma = y_hat.split(y_hat.shape[-1] // 2, dim=-1)
        
        with pyro.plate("data", y_hat.shape[-2]):
            y_hat = pyro.sample(
                "obs", 
                pyro.distributions.Normal(
                    y_hat_mu.squeeze(-1), 
